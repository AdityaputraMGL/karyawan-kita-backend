...
class AlphaCheckService {
  constructor(prisma) {
    this.prisma = prisma;
    console.log("🤖 AlphaCheckService initialized");
  }
  ...
  async checkAlphaForDate(checkDate) {
    console.log("\n" + "=".repeat(60));
    console.log("🔍 ALPHA CHECK SERVICE");
    ...
    try {
      ...
      console.log(`❌ Employees without attendance: ${absentEmployees.length}`);
      ...
      console.log(`🏖️ Employees on approved leave: ${employeesOnLeave.size}`);
      ...
      for (const employee of absentEmployees) {
        ...
        if (employeesOnLeave.has(employee.employee_id)) {
          console.log(
            `  ✓ Skipping employee ${employee.employee_id} (${employee.nama_lengkap}) - On approved leave`
          );
          continue;
        }
        ...
        try {
          ...
          const alphaRecord = await this.prisma.attendance.create({
            data: {
              ...
              status: "alpa",
              keterangan:
                "Alpha - Tidak ada absensi tercatat (Auto-generated by system)",
              tipe_kerja: "WFO",
              lokasi_masuk: null,
              lokasi_pulang: null,
          ...
          console.log(
            `  ❌ ALPHA recorded for: ${employee.nama_lengkap} (ID: ${employee.employee_id})`
          );
        } catch (error) {
          console.error(
            `  ⚠️ Error creating alpha for employee ${employee.employee_id}:`,
            error.message
          );
        }
      }
      ...
      console.log("\n" + "=".repeat(60));
      console.log("✅ ALPHA CHECK COMPLETED");
      ...
      console.error("❌ Error in alpha check:", error);
      return {
        success: false,
        error: error.message,
      };
   ...
   */
  setupCronJob() {
    console.log("⏰ Setting up Alpha Check Cron Job...");
    console.log("   Schedule: Every day at 18:00 WIB"); // ← UBAH INI

    // Cron pattern: "0 18 * * *"
    cron.schedule(
      "0 18 * * *", // ← UBAH INI (jam 18:00)
      async () => {
        console.log(
          "\n🤖 [CRON] Alpha check triggered at:",
          new Date().toISOString()
        );

        try {
          await this.checkYesterdayAlpha();
        } catch (error) {
          console.error("❌ [CRON] Error during alpha check:", error);
        }
      },
      {
        scheduled: true,
    ...
    console.log("   Next run: Tomorrow at 18:00 WIB"); // ← UBAH INI
  }
  ...
  async manualCheckAlpha(daysAgo = 1) {
    ...
    console.log(
      `\n🔧 Manual alpha check triggered for: ${
  ...
  async getAlphaStats(startDate, endDate) {
    try {
      ...
      console.log("📊 Fetching alpha statistics...");
      console.log(
  ...
  async getEmployeeAlphaCount(employeeId, startDate, endDate) {
    try {
      ...
      console.error("❌ Error getting employee alpha count:", error);
      throw error;
    }
  }
  ...
  async deleteAlphaRecord(attendanceId) {
    try {
      ...
      console.log(`✅ Alpha record deleted: ID ${attendanceId}`);
      return true;
    } catch (error) {
  ...
  async convertAlphaStatus(attendanceId, newStatus, keterangan = "") {
    try {
      ...
      console.log(`✅ Alpha converted to ${newStatus}: ID ${attendanceId}`);
      return updated;
    } catch (error) {
